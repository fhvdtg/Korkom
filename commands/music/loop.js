...
module.exports = {
   ...
   async execute({ inter }) {
        ...
        const errorMessage = await Translate(`Fma haja ghalta<${inter.member}>... Aawed jareb ? <❌>`);
        let baseEmbed = new EmbedBuilder()
            .setColor('#2f3136');

        if (!queue?.isPlaying()) return inter.editReply({ content: await Translate(`Mfma hata ghneya tekhdem <${inter.member}>... Aawed jareb ? <❌>`) });

        switch (inter.options._hoistedOptions.map(x => x.value).toString()) {
            case 'enable_loop_queue': {
                if (queue.repeatMode === QueueRepeatMode.TRACK) return inter.editReply({ content: `Lezmk kbal tnahel el ghneya ml loop mode (\`/loop Disable\`) ${inter.member}... Aawed jareb ? ❌` });
