...
module.exports = {
    ...
    async execute({ inter }) {
        ...
        if (channel.type !== ChannelType.GuildText) return inter.editReply({ content: await Translate(`Lezmk tabaathha fi text channel <❌>`) });

        const embed = new EmbedBuilder()
            .setTitle(await Translate('Controli ghneytk bl buttons louta !'))
            .setImage(inter.guild.iconURL({ size: 4096, dynamic: true }))
            .setColor('RANDOM')
            .setFooter({ text: await Translate('Ahla Mariem Dabbegh'), iconURL: inter.member.avatarURL({ dynamic: true }) });

        inter.editReply({ content: await Translate(`Baatht controller lil <${channel}>... <✅>`) });
